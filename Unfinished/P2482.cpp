...
int atk(int cur){
	...
	for(int nxt = a[cur].nxt;nxt != cur;nxt = a[nxt].nxt)
		if(!a[nxt].dead)
			if((a[nxt].iden == 2 and a[nxt].perfo == 1) or (a[cur].iden == 0 and a[nxt].perfo == -1))
				return nxt;
	return -1;
}

void pend(int x,int y){
	if(a[x].iden == 0 and a[y].iden == 1){
...
void respond_peach(int cur,int user){
	...
	if(cur == user){
		re = 0;
		for(int i = 1;i <= a[cur].cnt;++i)
			if(used[i] != rounds and a[cur].cards[i] == 'P'){
				re = i;
				break;
			}
			if(re){
...
bool respond_kill(int cur,int user){
	...
	if(cur == user){
		re = 0;
		for(int i = 1;i <= a[cur].cnt;++i)
			if(used[i] != rounds and a[cur].cards[i] == 'K'){
				re = i;
				break;
			}
		if(re)
...
bool respond_wuxie(int cur,int user){
	...
	if(cur == user){
		re = 0;
		for(int i = 1;i <= a[cur].cnt;++i)
			if(used[i] != rounds and a[cur].cards[i] == 'J'){
				re = i;
				break;
			}
			if(re)
...
void change_link(int cur){
	for(int pre = 1;pre <= n;++pre)
		if(!a[pre].dead and a[pre].nxt == cur){
			a[pre].nxt = a[cur].nxt;
			break;
		}
}
...
void do_fight(int cur,int aim,int user){
	...
	while(true){
		if(a[cur].iden == 0 and a[aim].iden == 1){
			lose_blood(aim,user);
			if(a[aim].bloods < 1){
				deadfan += (a[aim].iden == 2);
				a[aim].dead = 1;
				change_link(aim);
			}
			if(fanzhu == deadfan or a[1].dead)
			...
			if(used[i] != rounds){
				...
				switch(now){
					case 'P':
						...
						break;
					case 'K':
						if((!totkill or a[cur].equip) and ought(cur)){
							do_kill(cur);
							used[i] = rounds;
							++cntused;
							++totkill;
...
void _duel(){
	for(int i = 1,event = 0;!event and fanzhu > 0;i = a[i].nxt)
		if(!a[i].dead)
			event = playing(i);
}
